...
export default class Alert extends EnemyState {
    ...
    onEnter(options: Record<string, any>): void {
        ...
        let targetPos = new Vec2(options.target.x, options.target.y);
        this.path = this.owner.getScene().getNavigationManager().getPath(hw3_Names.NAVMESH, this.owner.position, targetPos)
    }
