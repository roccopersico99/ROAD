...
export default class GameLevel extends Scene {
    ...
    initScene(init: Record<string, any>): void {
        GameLevel.livesCount = init.lives;
        if(init.coins == undefined){
            GameLevel.coinCount = 0;
        }
        else{
            GameLevel.coinCount = init.coins;
        }
    }
                ...
                case HW4_Events.LEVEL_END:
                    {
                        ...
                        if(this.nextLevel){
                            ...
                            this.sceneManager.changeToScene(this.nextLevel, {lives: GameLevel.livesCount, coins: GameLevel.coinCount}, sceneOptions);
                        }
                        else{
    ...
    */
    ...
    protected handlePlayerEnemyCollision(player: AnimatedSprite, enemy: AnimatedSprite) {
        if(enemy.imageId === "GhostBunny"){
            if(player.position.y == enemy.position.y || player.position.y >= enemy.position.y){
                Input.disableInput();
                player.tweens.play("180");
                player.animation.play("DIE", false, "PlayerDied");
            }
                ...
                enemy.animation.play("DYING", false, "EnemyDied");
            }
        }
            ...
            else if(player.position.y > enemy.position.y -1){
